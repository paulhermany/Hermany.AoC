...
{
    public class _19 : ISolution
    {
        public string P1Assertion { get => "33"; }
        public string P2Assertion { get => "3472"; }
        ...
        */
        ...
        {
            var blueprints = input.Select(line => Regex.Matches(line, @"\s(\d+)").Select(_ => int.Parse(_.Value)).ToArray()).Select(_ => new Blueprint
            {
                Id = _[0],
                OreRobotCost = (_[1], 0, 0),
                ClayRobotCost = (_[2], 0, 0),
                ObsidianRobotCost = (_[3], _[4], 0),
                GeodeRobotCost = (_[5], 0, _[6])
            }).ToList();

            return blueprints.Select((b, i) => GetMaxGeodes(b, 24) * (i + 1)).Sum().ToString();
        }
        ...
        */
        public string P2(string[] input)
        {
            ...
            return blueprints.Select((b, i) => GetMaxGeodes(b, 32)).Aggregate((a,b) => a * b).ToString();
        }

        public int GetMaxGeodes(Blueprint blueprint, int timeRemaining)
        {
            var visited = new HashSet<((int ore, int clay, int obsidian, int geode) resources, (int ore, int clay, int obsidian, int geode) robots, int remaining)>();
            var queue = new Queue<((int ore, int clay, int obsidian, int geode) resources, (int ore, int clay, int obsidian, int geode) robots, int remaining)>();
            queue.Enqueue(((0, 0, 0, 0), (1, 0, 0, 0), timeRemaining));

            int maxGeodes = 0;

            while(queue.TryDequeue(out var current))
            {
                maxGeodes = Math.Max(maxGeodes, current.resources.geode);
                if (current.remaining == 0) continue;

                if (visited.Contains(current)) continue;
                visited.Add(current);

                if (CanProduce(blueprint.GeodeRobotCost, current.resources))
                {
                    queue.Enqueue((
                        GetResources(blueprint.GeodeRobotCost, current.resources, current.robots),
                        (current.robots.ore, current.robots.clay, current.robots.obsidian, current.robots.geode + 1),
                        current.remaining - 1
                    ));
                }
                else
                {
                    // only build obsidian robots if the current obsidian rate is less than the cost of geode bots
                    if (CanProduce(blueprint.ObsidianRobotCost, current.resources) && current.robots.obsidian < blueprint.GeodeRobotCost.obsidian)
                    {
                        queue.Enqueue((
                            GetResources(blueprint.ObsidianRobotCost, current.resources, current.robots),
                            (current.robots.ore, current.robots.clay, current.robots.obsidian + 1, current.robots.geode),
                            current.remaining - 1
                        ));
                    }

                    // only build clay robots if the current clay rate is less than the cost of obsidian bots
                    if (CanProduce(blueprint.ClayRobotCost, current.resources) && current.robots.clay < blueprint.ObsidianRobotCost.clay)
                    {
                        queue.Enqueue((
                            GetResources(blueprint.ClayRobotCost, current.resources, current.robots),
                            (current.robots.ore, current.robots.clay + 1, current.robots.obsidian, current.robots.geode),
                            current.remaining - 1
                        ));
                    }

                    // only build ore robots if the current ore rate is less than the maximum ore cost of geode, obsidian, and clay bots
                    var maxOreCost = Math.Max(blueprint.GeodeRobotCost.ore, Math.Max(blueprint.ObsidianRobotCost.ore, Math.Max(blueprint.ClayRobotCost.ore, blueprint.OreRobotCost.ore)));
                    if (CanProduce(blueprint.OreRobotCost, current.resources) && current.robots.ore < maxOreCost)
                    {
                        queue.Enqueue((
                            GetResources(blueprint.OreRobotCost, current.resources, current.robots),
                            (current.robots.ore + 1, current.robots.clay, current.robots.obsidian, current.robots.geode),
                            current.remaining - 1
                        ));
                    }
                    ...
                    // gather resources
                    queue.Enqueue((
                        GetResources((0, 0, 0), current.resources, current.robots),
                        current.robots,
                        current.remaining - 1
                    ));
                }
        ...
        public (int ore, int clay, int obsidian, int geode) GetResources(
            (int ore, int clay, int obsidian) cost,
            (int ore, int clay, int obsidian, int geode) resources,
            (int ore, int clay, int obsidian, int geode) robots
        ) => (
                resources.ore - cost.ore + robots.ore,
                resources.clay - cost.clay + robots.clay,
                resources.obsidian - cost.obsidian + robots.obsidian,
    ...
    {
        public int Id;
        public (int ore, int clay, int obsidian) OreRobotCost;
        public (int ore, int clay, int obsidian) ClayRobotCost;
        public (int ore, int clay, int obsidian) ObsidianRobotCost;
        public (int ore, int clay, int obsidian) GeodeRobotCost;
    }
}
