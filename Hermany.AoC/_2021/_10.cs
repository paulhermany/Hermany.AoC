...
namespace Hermany.AoC._2021
{
    ...
    {
        public string P1Assertion { get => "26397"; }
        public string P2Assertion { get => "288957"; }
        ...
        public string P1(string[] input)
        {
            ...
            for(var lineIndex = 0; lineIndex < input.Length; lineIndex++)
            {
                ...
                {
                    if (stack.Count > 0 && map[stack.Peek()] == line[i])
                    {
                        stack.Pop();
                        continue;
                    }

                    if (map.Keys.Contains(line[i]))
                    {
                        stack.Push(line[i]);
                        continue;
                    }
            ...
            for (var lineIndex = 0; lineIndex < input.Length; lineIndex++)
            {
                ...
                if(!corrupt)
                {
                    ...
                    while (stack.TryPop(out c))
                        score = score * 5 + points[map[c]];
                    scores.Add(score);
                }
            }

            var ordered = scores.OrderBy(_ => _);

            return scores.OrderBy(_ => _).Skip(scores.Count / 2).First().ToString();
        }
    }
}
