...
namespace Hermany.AoC._2021
{
    public class _03 : ISolution
    {
        ...
        */
        ...
        {
            var bits = new Dictionary<char, Dictionary<int, int>>();
            bits.Add('0', new Dictionary<int, int>());
            bits.Add('1', new Dictionary<int, int>());
            ...
            for(var i = 0; i < input[0].Length; i++)
            {
                gamma.Append(bits['0'][i] > bits['1'][i] ? '0' : '1');
                epsilon.Append(bits['0'][i] < bits['1'][i] ? '0' : '1');
            }
        ...
        */
        ...
        private int GetOxygenGeneratorRating(string[] input)
        {
            ...
            for (var j = 0; j < input[0].Length; j++)
            {
                ...
                {
                    var bit = int.Parse(input[i][j].ToString());
                    if (input[i].StartsWith(filter))
                        counts[bit]++;
                }

                filter = string.Concat(filter, counts[0] > counts[1] ? "0" : "1");
            }

            return Convert.ToInt32(filter, 2);
        }
        ...
        private int GetCO2ScrubberRating(string[] input)
        {
            ...
            for (var j = 0; j < input[0].Length; j++)
            {
                ...
                if (counts[0] == 0 && counts[1] == 1)
                    ...
                    nextBit = counts[0] <= counts[1] ? "0" : "1";

                filter = string.Concat(filter, nextBit);
            }
